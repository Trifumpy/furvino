...
import { useCreateNovelGalleryItem, useDeleteNovelGalleryItem } from "@/novels/hooks";
import { useNovel } from "@/novels/providers";
import { toast } from "react-toastify";
import { NovelGallery } from "@/novels/components";
  ...
  return (
    <Stack gap={1}>
      ...
      <Stack direction={{ xs: "column", md: "row" }} flexWrap="wrap" gap={2}>
        {Array.from({ length: MAX_GALLERY_ITEMS }, (_, i) => i + 1).map((slot) => {
                ...
                {existing ? (
                  <Stack gap={1}>
                    ...
                    <IconButton
                      ...
                      onClick={async () => {
                        await deleteGalleryItem({ novelId: novel.id, galleryItemId: existing.id });
                        // Optimistically update local state so the slot reverts to uploader
                        novel.galleryItems = novel.galleryItems.filter((gi) => gi.id !== existing.id);
                        setRevision((r) => r + 1);
                  ...
                  <ImageInput
                    ...
                    onUpload={async (file) => {
                      ...
                      if (uploadingSlot !== null) {
                        toast.info("Another gallery upload is in progress. Please wait.");
                        return;
                      }
                      try {
                        setUploadingSlot(slot);
                        const created = await createGalleryItem({ novelId: novel.id, galleryItemFile: file, slot });
                        ...
                        novel.galleryItems = [
                          ...
                          { id, imageUrl: url, createdAt: new Date().toISOString(), footer: null },
                        ];
