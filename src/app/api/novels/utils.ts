...
export async function enrichToFullNovel(data: Novel): Promise<FullNovel> {
  ...
  return {
    ...
    foregroundOpacityPercent: (data as unknown as { foregroundOpacityPercent?: number | null }).foregroundOpacityPercent ?? 80,
    foregroundBlurPercent: (data as unknown as { foregroundBlurPercent?: number | null }).foregroundBlurPercent ?? 20,
...
export async function updateNovelAndEnrich(
  ...
  const updated = await prisma.novel.update({
    ...
    data: {
      ...
      foregroundOpacityPercent: (data as unknown as { foregroundOpacityPercent?: number | null }).foregroundOpacityPercent ?? undefined,
      foregroundBlurPercent: (data as unknown as { foregroundBlurPercent?: number | null }).foregroundBlurPercent ?? undefined,
