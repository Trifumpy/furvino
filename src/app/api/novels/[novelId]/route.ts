...
export const PUT = wrapRoute<UpdateNovelParams>(async (request, { params }) => {
  ...
  const updatedNovel = await prisma.novel.update({
    ...
    data: {
      ...
      foregroundOpacityPercent: (rest as unknown as { foregroundOpacityPercent?: number | null }).foregroundOpacityPercent ?? 80,
      foregroundBlurPercent: (rest as unknown as { foregroundBlurPercent?: number | null }).foregroundBlurPercent ?? 20,
      foregroundColorHex: (rest as unknown as { foregroundColorHex?: string | null }).foregroundColorHex,
      foregroundTextColorHex: (rest as unknown as { foregroundTextColorHex?: string | null }).foregroundTextColorHex,
      buttonBgColorHex: (rest as unknown as { buttonBgColorHex?: string | null }).buttonBgColorHex,
      tags: rest.tags,
