...
import { UpdateCollectionBody, updateCollectionSchema } from "@/contracts/collections";
...
export const GET = wrapRoute<Params>(async (_req, { params }: NextParams<Params>) => {
  const { clerkId } = await ensureClerkId();
  const user = await getOrCreateUserByExternalId(clerkId);
  const { collectionId } = await params;
  ...
  const novels: ListedNovel[] = data.items.map((ci) =>
    enrichNovelWithAuthor({ ...ci.novel, author: ci.novel.author })
  );
...
export const PUT = wrapRoute<Params>(async (req, { params }: NextParams<Params>) => {
  const { clerkId } = await ensureClerkId();
  const user = await getOrCreateUserByExternalId(clerkId);
  const { collectionId } = await params;
  const body = await validateRequestBody<UpdateCollectionBody>(req, updateCollectionSchema);
  ...
  const updated = await prisma.collection.update({
    ...
    data: {
      ...
      isPublic: body.hasOwnProperty("isPublic") ? (body as unknown as { isPublic?: boolean }).isPublic ?? false : undefined,
    },
